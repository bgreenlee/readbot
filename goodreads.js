...
//const http = require('https'); // import http from 'http';
const request = require('request');
const xml2js = require('xml2js'); // import xml2js from 'xml2js';
const oauth = require('oauth'); //import {OAuth as oauth} from 'oauth';
...
class Goodreads {
  ...
  showUser(username) {
    this.options.url = `${this.options.endpoint}/user/show.xml?key=${this.options.key}&username=${username}`
  ...
  getAuthor(authorId, page) {
    ...
    this.options.url = `${this.options.endpoint}/author/list/${authorId}?key=${this.options.key}&page=${page}`;
    return new Promise((resolve, reject) => {
  ...
  getSeries(seriesId) {
    // Provide path to the API
    this.options.url = `${this.options.endpoint}/series/${seriesId}?key=${this.options.key}`
    return new Promise((resolve, reject) => {
  ...
  getShelves(userId, callback) {
    // Provide path to the API
    this.options.url = `${this.options.endpoint}/shelf/list.xml?user_id=${userId}&key=${this.options.key}`
    return new Promise((resolve, reject) => {
  ...
  getSingleShelf(shelfOptions, callback) {
    ...
    this.options.url = `${this.options.endpoint}/review/list/${userID}.xml?${querystring.stringify(shelfOptions)}`;
  ...
  getFriends(userId, accessToken, accessTokenSecret) {
    // Provide path to the API
    this.options.url = `${this.options.endpoint}/friend/user/${userId}?format=xml`;
  ...
  searchBooks(q) {
    this.options.url = `${this.options.endpoint}/search/index.xml?key=${this.options.key}&q=${encodeURI(q)}`;
    console.log("sending query:",this.options.url);
    return new Promise((resolve, reject) => {
      this.getRequest().then(e => {
        console.log("e:", e);
        resolve(e.GoodreadsResponse.search[0])
      }, reject)
    })
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      oa.getOAuthAccessToken(oauthToken, oauthTokenSecret, authorize, (error, oauthAccessToken, oauthAccessTokenSecret, results) => {
        ...
        if (error) {
          ...
          oa.get('https://www.goodreads.com/api/auth_user', oauthAccessToken, oauthAccessTokenSecret, (error, data, response) => {
            if (error) {
  ...
  getRequest() {
    return new Promise((resolve, reject) => {
      ...
      let parser = new xml2js.Parser()
      console.log("getRequest:",_options);
      return request(_options, (error, res, body) => {
        if (error) {
          return reject(error);
        }
        console.log("got body:", body);
        parser.parseString(body, result => resolve(result));
      });
   });
  }
  ...
  getProtectedRequest() {
    return new Promise((resolve, reject) => {
      let oa = new oauth(this.options.oauth_request_url, this.options.oauth_access_url, this.options.key, this.options.secret, this.options.oauth_version, this.options.callback, this.options.oauth_encryption);
      let parser = new xml2js.Parser();
      oa.get(this.options.url, this.oauthAccessToken, this.oauthAcessTokenSecret, (error, data, response) => {
        if (error) {
          reject(`Error getting OAuth request token : ${JSON.stringify(error)}`, 500);
        } else {
          return parser.parseString(data);
...
module.exports = {
  client: function(options) { return new Goodreads(options) }
}
